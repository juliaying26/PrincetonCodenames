  ...
  def get_clue(self):
    ...
    for curr_clue, curr_freq in clues:
      if _acceptable(curr_clue):
        ...
        # TESTING
        total = 0
        for word in clue_cards:
          sim = model.wv.similarity(word, curr_clue)
          total += abs(sim)
        ave_sim_target = total/len(clue_cards)
        total = 0
        for word in problematic_neg:
          sim = model.wv.similarity(word, curr_clue)
          total += abs(sim)
        try: ave_sim_prob = total/len(problematic_neg)
        except: ave_sim_prob = None
        total = 0
        for word in all_neg:
          sim = model.wv.similarity(word, curr_clue)
          total += abs(sim)
        ave_sim_all_bad = total/len(all_neg)

        return curr_clue, len(clue_cards), ave_sim_target, ave_sim_prob, ave_sim_all_bad 
...
header = ['clue_sizes', 'target_sims', 'problem_sims', 'all_bad_sims']
clue_sizes = ['clue_sizes']
target_sims = ['target_sims']
problem_sims = ['problem_sims']
all_bad_sims = ['all_bad_sims']

import csv

for i in range(x):
    board = CodenamesBoard(threshold)
    _,size,ave_sim_target, ave_sim_prob, ave_sim_all_bad = board.get_clue()
    clue_sizes.append(size) 
    target_sims.append(ave_sim_target)
    problem_sims.append(ave_sim_prob)
    all_bad_sims.append(ave_sim_all_bad)

with open(f'quant_data_threshold{threshold}.csv', 'w') as f:
  writer = csv.writer(f)
  writer.writerow(clue_sizes)
  writer.writerow(target_sims)
  writer.writerow(problem_sims)
  writer.writerow(all_bad_sims)